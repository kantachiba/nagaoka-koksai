    ...
    return (
      <section className="rounded-card bg-white p-6 border-2 border-faded-gray">
        ...
        <div className="mt-5 space-y-5">
          {scope.kind === 'admin' && (
            <div>
              <label className="mb-1.5 block text-xs font-bold text-pencil-gray">団体</label>
              <select
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              <label className="mb-1.5 block text-xs font-bold text-pencil-gray">実施日</label>
              <input type="date" value={draft.heldOn}
                onChange={(e) => setDraft({ ...draft, heldOn: e.target.value })}
                className="w-full rounded-card border border-faded-gray px-3 py-2.5 text-sm" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-bold text-pencil-gray">公開日</label>
              <input type="date" value={draft.publishedAt}
                onChange={(e) => setDraft({ ...draft, publishedAt: e.target.value })}
                className="w-full rounded-card border border-faded-gray px-3 py-2.5 text-sm" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-bold text-pencil-gray">参加者数（任意）</label>
              ...
              {tags.map((tag) => {
                ...
                return (
                  <button key={tag.id} type="button"
                    ...
                    aria-pressed={on}
                    className={`rounded-card px-3.5 py-1.5 text-sm font-semibold border-2 ${
          ...
          <div>
            ...
            <input value={draft.slug}
              ...
              className="w-full rounded-card border border-faded-gray px-3 py-2.5 font-mono text-sm" />
          ...
          {reports.map((report) => (
            <li key={report.id} className="flex flex-wrap items-center gap-3 py-3">
              <span className={`rounded-card px-2 py-0.5 text-xs font-bold ${
                report.status === 'published' ? 'bg-emerald-50 text-emerald-800' : 'bg-paper-white text-pencil-gray'
              }`}>
                {report.status === 'published' ? '公開中' : '下書き'}
              </span>
              <span className="min-w-0 flex-1">
                <span className="block truncate font-medium text-charcoal">{report.title.ja || '(無題)'}</span>
